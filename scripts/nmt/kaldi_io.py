...
# This code has been modified. Portions copyright 2017 Amazon.com, Inc. or its
# affiliates. Please see LICENSE.txt for applicable license terms and NOTICE.txt
# for applicable notices.
...
def read_ali_ark_new(line):
    (key, ints) = line.split(' ', 1)
    arr = ints.strip().split()
...
def read_vec_int(file_or_fd):
  ...
  if binary == b'\0B': # binary flag
    ...
    # zhiheng, bug fix for case 46uc030b 2\n
...
def write_vec_int(file_or_fd, v, key=''):
  ...
  try:
    if key != '' : fd.write(str.encode(key+' ')) # ark-files have keys (utterance-id),
    fd.write(b'\0B') # we write binary!
    # dim,
    fd.write(b'\4') # int32 type,
...
# Reading,
def read_mat_scp_new(line):
    (key,rxfile, seqlen) = line.split(' ')
    mat = read_mat(rxfile)
    return key,mat
# Reading,
...
# convert ark file to scp file, mainly used to cnovert CTC model ouput
def convert_ark_to_scp(file_or_fd, output_scp, sort=True):
  res = []
  fd = open_or_fd(file_or_fd)
  try:
    key = read_key(fd)
    while key:
      ...
      # import pdb; pdb.set_trace()
      mat = read_mat(fd)
      mat_new = mat[np.argmax(mat, axis=1) != 0, :]
      res.append([key, file_or_fd + ":" + str(offset), mat_new.shape[0]])
      key = read_key(fd)
  finally:
    if fd is not file_or_fd : fd.close()
  ...
  writer = open(output_scp, 'w')
  for ele in res:
    writer.write(ele[0] + " " + ele[1] + " " + str(ele[2])+ "\n")
  writer.close()
...
def _read_mat_binary(fd):
  ...
  # https://github.com/vesis84/kaldi-io-for-python/pull/7
  if dtype.startswith(b'CM'): return _read_compressed_mat(fd, dtype)
  if dtype == b'FM ': sample_size = 4 # floats
  if dtype == b'DM ': sample_size = 8 # doubles
  assert(sample_size > 0)
  # Dimensions
  fd.read(1)
...
def _read_compressed_mat(fd, format):
  ...
  global_header = np.dtype([('minvalue', 'float32'), ('range', 'float32'), ('num_rows', '<i4'),
                            ('num_cols', '<i4')])  # member '.format' is not written,
